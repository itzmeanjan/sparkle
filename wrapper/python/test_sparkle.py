...
def test_schwaemm256_256_kat():
    """
    Tests functional correctness of Schwaemm256-256 AEAD implementation, using
    Known Answer Tests submitted along with final round submission of Sparkle in NIST LWC

    See https://csrc.nist.gov/projects/lightweight-cryptography/finalists
    """

    from sparkle import schwaemm256_256_encrypt, schwaemm256_256_decrypt

    with open("LWC_AEAD_KAT_256_256.txt", "r") as fd:
        while True:
            cnt = fd.readline()
            if not cnt:
                # no more KATs remaining
            ...
            cipher, tag = schwaemm256_256_encrypt(key, nonce, ad, pt)
            flag, text = schwaemm256_256_decrypt(key, nonce, tag, ad, cipher)

            assert (
                cipher + tag == ct
            ), f"[Schwaemm256-256 KAT {cnt}] expected cipher to be 0x{ct.hex()}, found 0x${(cipher + tag).hex()} !"
            assert (
                pt == text and flag
            ), f"[Schwaemm256-256 KAT {cnt}] expected plain text 0x{pt.hex()}, found 0x{text.hex()} !"
