...
def schwaemm256_256_encrypt(
    key: bytes, nonce: bytes, data: bytes, text: bytes
) -> Tuple[bytes, bytes]:
    """
    Encrypts M ( >=0 ) -many plain text bytes, while using 32 -bytes secret key,
    32 -bytes public message nonce & N ( >=0 ) -bytes associated data, while producing
    M -bytes cipher text & 32 -bytes authentication tag ( in order )
    """
    assert len(key) == 32, "Schwaemm256-256 takes 32 -bytes secret key !"
    assert len(nonce) == 32, "Schwaemm256-256 takes 32 -bytes nonce !"
    ...
    text_ = np.frombuffer(text, dtype=u8)
    enc = np.empty(ct_len, dtype=u8)
    tag = np.empty(32, dtype=u8)

    args = [uint8_tp, uint8_tp, uint8_tp, len_t,
            uint8_tp, uint8_tp, len_t, uint8_tp]
    SO_LIB.schwaemm256_256_encrypt.argtypes = args

    SO_LIB.schwaemm256_256_encrypt(key_, nonce_, data_, ad_len,
                                   text_, enc, ct_len, tag)
...
def schwaemm256_256_decrypt(
    key: bytes, nonce: bytes, tag: bytes, data: bytes, enc: bytes
) -> Tuple[bool, bytes]:
    """
    Decrypts M ( >=0 ) -many cipher text bytes, while using 32 -bytes secret key,
    32 -bytes public message nonce, 32 -bytes authentication tag & N ( >=0 ) -bytes
    associated data, while producing boolean flag denoting verification status ( which 
    must hold truth value, check before consuming decrypted output bytes ) &
    M -bytes plain text ( in order )
    """
    assert len(key) == 32, "Schwaemm256-256 takes 32 -bytes secret key !"
    assert len(nonce) == 32, "Schwaemm256-256 takes 32 -bytes nonce !"
    assert len(tag) == 32, "Schwaemm256-256 takes 32 -bytes authentication tag !"
    ...
    args = [uint8_tp, uint8_tp, uint8_tp,
            uint8_tp, len_t, uint8_tp, uint8_tp, len_t]
    SO_LIB.schwaemm256_256_decrypt.argtypes = args
    SO_LIB.schwaemm256_256_decrypt.restype = bool_t

    f = SO_LIB.schwaemm256_256_decrypt(key_, nonce_, tag_, data_,
                                       ad_len, enc_, dec, ct_len)
