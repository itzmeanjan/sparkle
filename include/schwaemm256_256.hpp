...
)
{
  ...
  initialize<32ul, 32ul, 8ul, 12ul>(state, key, nonce);

  if (d_len > 0) {
    process_data<32ul, 32ul, A0, A1, 8ul, 8ul, 12ul>(state, data, d_len);
  }
  if (ct_len > 0) {
    process_text<32ul, 32ul, M0, M1, 8ul, 8ul, 12ul>(state, txt, enc, ct_len);
  }

  finalize<32ul, 32ul>(state, key, tag);
}
...
)
{
  ...
  if (ct_len > 0) {
    process_cipher<32ul, 32ul, M0, M1, 8ul, 8ul, 12ul>(state, enc, dec, ct_len);
  }

  finalize<32ul, 32ul>(state, key, tag_);
