...
)
{
  ...
  initialize<32ul, 16ul, 6ul, 11ul>(state, key, nonce);

  if (d_len > 0) {
    process_data<32ul, 16ul, A0, A1, 6ul, 7ul, 11ul>(state, data, d_len);
  }
  if (ct_len > 0) {
    process_text<32ul, 16ul, M0, M1, 6ul, 7ul, 11ul>(state, txt, enc, ct_len);
  }

  finalize<32ul, 16ul>(state, key, tag);
}
...
)
{
  ...
  if (ct_len > 0) {
    process_cipher<32ul, 16ul, M0, M1, 6ul, 7ul, 11ul>(state, enc, dec, ct_len);
  }

  finalize<32ul, 16ul>(state, key, tag_);
